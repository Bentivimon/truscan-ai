...
import time
from optimizers.CustomAdamV2 import CustomAdamV2, CustomAdamV3, CustomAdamV4, CustomAdamV5, CustomAdamV6, HybridOptimizer, AdamM_Hessian
...
facenet_model.compile(optimizer='adam', loss='binary_crossentropy', metrics=['accuracy'])
...
print(f"Shape of X_train_single_frame: {X_train_single_frame.shape}")
print(f"Shape of X_val_single_frame: {X_val_single_frame.shape}")

# Використання раннього стопу та динамічної зміни швидкості навчання
early_stopping = EarlyStopping(monitor='val_loss', patience=15, restore_best_weights=True)
reduce_lr = ReduceLROnPlateau(monitor='val_loss', factor=0.5, patience=3, min_lr=1e-6, verbose=1)

# Створюємо окремий екземпляр оптимізатора для кожної моделі
optimizer_resnet = Adam(learning_rate=1e-4)
optimizer_efficientnet = Adam(learning_rate=1e-4)
optimizer_xception = Adam(learning_rate=1e-4)
optimizer_inception = Adam(learning_rate=1e-4)
optimizer_facenet = Adam(learning_rate=1e-4)

# Компіляція моделей з окремими оптимізаторами
resnet_model.compile(optimizer=optimizer_resnet, loss='binary_crossentropy', metrics=['accuracy'])
efficientnet_model.compile(optimizer=optimizer_efficientnet, loss='binary_crossentropy', metrics=['accuracy'])
xception_model.compile(optimizer=optimizer_xception, loss='binary_crossentropy', metrics=['accuracy'])
inception_model.compile(optimizer=optimizer_inception, loss='binary_crossentropy', metrics=['accuracy'])
facenet_model.compile(optimizer=optimizer_facenet, loss='binary_crossentropy', metrics=['accuracy'])

csv_logger_restnet = CSVLogger('training_log_restnet.csv', append=True)
csv_logger_efficientnet = CSVLogger('training_log_efficientnet.csv', append=True)
csv_logger_xception = CSVLogger('training_log_xception.csv', append=True)
csv_logger_inception = CSVLogger('training_log_inception.csv', append=True)
csv_logger_facenet = CSVLogger('training_log_facenet.csv', append=True)

# Тренування моделей з використанням раннього стопу і зменшення швидкості навчання
resnet_model.fit(X_train_single_frame, y_train, epochs=50, batch_size=8,
                 validation_data=(X_val_single_frame, y_val),
                 callbacks=[early_stopping, reduce_lr, csv_logger_restnet])

efficientnet_model.fit(X_train_single_frame, y_train, epochs=50, batch_size=8,
                       validation_data=(X_val_single_frame, y_val),
                       callbacks=[early_stopping, reduce_lr, csv_logger_efficientnet])

xception_model.fit(X_train_single_frame, y_train, epochs=50, batch_size=8,
                   validation_data=(X_val_single_frame, y_val),
                   callbacks=[early_stopping, reduce_lr, csv_logger_xception])

inception_model.fit(X_train_single_frame, y_train, epochs=50, batch_size=8,
                    validation_data=(X_val_single_frame, y_val),
                    callbacks=[early_stopping, reduce_lr, csv_logger_inception])

facenet_model.fit(X_train_single_frame, y_train, epochs=50, batch_size=8,
                  validation_data=(X_val_single_frame, y_val),
                  callbacks=[early_stopping, reduce_lr, csv_logger_facenet])
...
for num_frames in candidate_num_frames:
    ...
    initial_weights = np.full((train_meta_features.shape[1],), 1 / train_meta_features.shape[1])
    constraints = {'type': 'eq', 'fun': lambda w: sum(w) - 1}  # Сума ваг = 1
    bounds = [(0, 1)] * train_meta_features.shape[1]
...
# Збереження архітектури моделей у JSON
def save_model_to_json(model, filename):
    model_json = model.to_json()
    with open(filename, "w") as json_file:
        json.dump(model_json, json_file)
    print(f"Архітектуру моделі збережено в {filename}")
...
save_model_to_json(resnet_model, f"{save_dir}/resnet_model.json")
save_model_to_json(efficientnet_model, f"{save_dir}/efficientnet_model.json")
save_model_to_json(xception_model, f"{save_dir}/xception_model.json")
save_model_to_json(inception_model, f"{save_dir}/inception_model.json")
save_model_to_json(facenet_model, f"{save_dir}/facenet_model.json")

# Збереження моделей у форматі HDF5
resnet_model.save(f"{save_dir}/resnet_model.h5")
efficientnet_model.save(f"{save_dir}/efficientnet_model.h5")
xception_model.save(f"{save_dir}/xception_model.h5")
inception_model.save(f"{save_dir}/inception_model.h5")
facenet_model.save(f"{save_dir}/facenet_model.h5")
