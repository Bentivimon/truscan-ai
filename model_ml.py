...
async def process_video_file(file: UploadFile = File(...)):
    ...
    with open('models/video-models/xception_model_json.json', 'r') as file:
        model_json = file.read().rstrip()
    model = model_from_json(model_json)  # Вибір моделі для прогнозування
    model.load_weights('models/video-models/xception_model.weights.h5')

    predict_result = predict_video(model, video_path)
